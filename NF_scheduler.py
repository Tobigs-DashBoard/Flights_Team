...
def make_flight_combi(target_regions, korea_airport_list, total_combi_length, time_gap=181):
    global progress
    for departure in korea_airport_list:
        for arrival_target in target_regions:
            for airport in request_airport_map[arrival_target]:
                ...
                # 국내가 아닌 경우 출발 <-> 도착을 바꿔서 하면 중복이 생겼으나, 현재는 출발 공항을 인천, 김포, 제주 3곳으로 줄였으므로 국내 인경우도 중복을 허용함
                # if not is_domestic:
                for i in range(1, time_gap):
                    target_date = today + timedelta(days=i)
...
def crawl_flights(departure, arrival, target_date, is_domestic, cnt=0):
    ...
    if is_domestic:
        if empty_combi_map[departure+arrival]<10:
            ...
            # 여서 fetch_domestic_flights를 15개 씩 병렬처리하고싶음 (15개 처리한 후에 다시 15개)
            if cnt==1:
                ...
                empty_combi_map[departure+arrival]=0
                end_time=time.time()
                logger.info(f"항공권 크롤링 소요시간:{round(end_time-start_time,2)}초")
...
if __name__ == "__main__":
    try:
        # time.sleep(60)
        target_regions = os.environ.get('TARGET_REGION', '대한민국').split(',') # 도착 지역 설정
        ...
        make_flight_combi(target_regions, korea_airport_list, total_combi_length, 181)
