# import json
import time
from Naver_Flights.NF_api_params import international_payload_form, return_header
from NF_functions import *
from NF_DB_params import get_db_connection, execute_db_query, query_dict
from multiprocessing import Process, Queue
from psycopg2 import sql

def save_flight_info(schedules, airline_map, departure, arrival, date, today, queue):
    '''비행 정보 저장'''
        ...
        for option, fare_list in values['fare'].items():
            ...
            for i in fare_list:
                try:
                    ...
                    execute_db_query(conn, cur, query, (
                        key, option,fetched_date, adult_fare, base_fare, naver_fare, tax, Qcharge, purchase_url
                    ))
...
def fetch_international_flights(departure, arrival, date, today, cnt):
    ...
    queue = Queue()
    p1 = Process(target=save_flight_info, args=(schedules, airline_map, departure, arrival, date,today, queue))
