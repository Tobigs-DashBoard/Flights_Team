...
def save_flight_info(schedules, airline_map, airport_map, departure, arrival, date, queue):
    ...
    # JSON 파일로 저장
    file_path = f'Naver_Results/일본/{arrival}/flight_info/{departure}_{arrival}_{date}.json'
...
def save_fare_info(fares, fare_types, departure, arrival, date, queue):
    ...
    # JSON으로 저장
    file_path = f'Naver_Results/일본/{arrival}/fare_info/{departure}_{arrival}_{date}.json'
    ensure_dir(file_path)
    with open(file_path, 'w', encoding='utf-8') as f:
...
def fetch_naver_flights_date(departure, arrival, date):
    # 요청 헤더
...
if __name__ == "__main__":
    ...
    for airport in request_airport_map['일본']:
        ...
        for i in range(1, 4):
            ...
            fetch_naver_flights_date(departure, arrival, formatted_date)