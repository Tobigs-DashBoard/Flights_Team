...
class Batch_Queue:
    ...
    def add_to_queue(self, queue_name, query_params):
        ...
        if queue_name=='flight_info' and len(self.air_id_check_set)==origin_check_length+1: # 기존과 다른 air_id가 추가되었다는 의미
            try:
                ...
                self.logger.error(f"flight_info 큐에 항목 추가 및 insert 중 오류 발생: {e}")
        
        elif len(self.air_id_check_set)==origin_check_length and (queue_name=='fare_info' or queue_name=='layover_info'): # fare_info나 layover는 air_id가 이미 등록되어있을 수 밖에 없음
            try:
                self.queue_dict[queue_name].append(query_params)
                self.total_length += 1
                if self.total_length >= self.batch_size:
                ...
                self.logger.error(f"fare_info 혹은 layover info 큐에 항목 추가 및 insert 중 오류 발생: {e}")
    ...
    def flush_queue(self, queue_name):
        ...
        if success_flag:
            ...
            self.logger.info(f"{queue_name} 큐를 성공적으로 플러시했습니다. (소요 시간 : {round(insert_end-insert_start, 2)}초)")
            return True
        else:
            self.logger.error(f"{queue_name} 큐 플러시 실패, 중복 요소 제거 후 재시도")
            success_flag = self.db_instance.execute_values_query(query, list(set(list(queue))))
            insert_end=time.time()
            self.queue_dict[queue_name].clear()
            if success_flag:
                self.logger.info(f"{queue_name} 큐를 성공적으로 플러시했습니다. (소요 시간 : {round(insert_end-insert_start, 2)}초)")
            else:
                self.logger.error(f"중복 제거 후에도 {queue_name} 큐를 플러시하지 못했습니다. (소요 시간 : {round(insert_end-insert_start, 2)}초)")
            return False
    ...
    def flush_total_queues(self):
        ...
        try:
            ...
            self.air_id_check_set=set()
            flush_end=time.time()
            self.logger.info(f"전체 큐 플러시 소요 시간 : {round(flush_end-flush_start, 2)}초")
            return True
        except Exception as e:
