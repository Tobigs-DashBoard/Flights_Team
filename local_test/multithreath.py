...
from NF_db_params_test import get_db_connection, execute_db_query, query_dict

def save_flight_info(schedules, airline_map, today):
    '''비행 정보 저장'''
    ...
    for schedule in schedules[0].values():
        ...
        for index, detail in enumerate(details):
            ...
            if index == len(details) - 1:
                layover_arrival_airport = airport_region_map[detail['ea']]['name']  # 도착 공항
...
def save_fare_info(fares, fare_types, today):
    conn = get_db_connection()  # db 연결
    cur = conn.cursor()  # DB 커서 객체 (insert, delete, select 등 명령어 수행)
    '''운임 정보 저장'''
    fetched_date = today.strftime('%Y%m%d')
...
def fetch_international_flights(departure, arrival, date, today, cnt):
    ...
    '''항공권 상세 정보, 운임 정보 파싱을 멀티 스레딩으로 분산 처리'''
    t1 = threading.Thread(target=save_flight_info, args=(schedules, airline_map, today))
    t2 = threading.Thread(target=save_fare_info, args=(fares, fare_types, today))
